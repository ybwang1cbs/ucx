                   ...
                   const ucp_request_param_t *param)
{
    ...
    if (ucs_unlikely(param->op_attr_mask & (UCP_OP_ATTR_FIELD_DATATYPE |
        ...
        ucs_print("NOT put_short, length=%zu", length);
                             ...
                             const ucp_request_param_t *param)
{
    ...
    if (worker->context->config.ext.proto_enable) {
        ...
        ret = ucp_proto_request_send_op(ep, &ucp_rkey_config(worker, rkey)->proto_select,
                                  rkey->cfg_index, req, UCP_OP_ID_PUT, buffer,
                                  count, ucp_dt_make_contig(1), count, param);
    } else {
                             ...
                             const ucp_request_param_t *param)
{
    ...
    if (worker->context->config.ext.proto_enable) {
        ...
        ret = ucp_proto_request_send_op(ep, &ucp_rkey_config(worker, rkey)->proto_select,
                                  rkey->cfg_index, req, UCP_OP_ID_GET, buffer,
                                  count, ucp_dt_make_contig(1), count, param);
    } else {
        status = UCP_RKEY_RESOLVE(rkey, ep, rma);
        if (status != UCS_OK) {
