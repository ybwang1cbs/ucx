                         ...
                         int print_cfg, ucp_worker_cfg_index_t *cfg_index_p)
{
    ...
    if (context->config.ext.proto_enable) {
        ...
        // TODO get the minimum of different flags (UCP_OP_ATTR_FLAG_FAST_CMPL on/off)
        ucp_proto_select_get_short(worker, &ep_config->proto_select, ep_cfg_index,
                                   UCP_WORKER_CFG_INDEX_NULL, UCP_OP_ID_TAG_SEND,
                                   0, UCP_PROTO_FLAG_AM_SHORT, &tag_short);

        // TODO remove ep_config->tag.max_eager_short and put 'ucp_proto_select_short_t'
        // struct in ep_config instead
        ep_config->tag.max_eager_short.memtype_off = tag_short.memtype_off_thresh;
        ep_config->tag.max_eager_short.memtype_on  = tag_short.memtype_on_thresh;
    }
                           ...
                           ucp_worker_cfg_index_t *cfg_index_p)
{
    ...
    // TODO get the minimum of different flags (UCP_OP_ATTR_FLAG_FAST_CMPL on/off)
    ucp_proto_select_get_short(worker, &rkey_config->proto_select,
