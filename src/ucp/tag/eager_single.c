...
static ucs_status_t
ucp_eager_bcopy_single_proto_progress(uct_pending_req_t *self)
{
    ucp_request_t                   *req = ucs_container_of(self, ucp_request_t,
                                                            send.uct);
    const ucp_proto_single_priv_t *spriv = req->send.proto_config->priv;
    ...
    return ucp_proto_am_bcopy_single_progress(req, UCP_AM_ID_EAGER_ONLY,
                                              spriv->super.lane,
                                              ucp_eager_single_pack, SIZE_MAX,
...
static ucp_proto_t ucp_eager_bcopy_single_proto = {
    ...
    .config_str = ucp_proto_single_config_str,
    .progress   = ucp_eager_bcopy_single_proto_progress,
};
...
static ucs_status_t ucp_eager_zcopy_single_progress(uct_pending_req_t *self)
{
    ...
    if (!(req->flags & UCP_REQUEST_FLAG_PROTO_INITIALIZED)) {
        status = ucp_proto_request_zcopy_init(req, UCS_BIT(spriv->reg_md),
